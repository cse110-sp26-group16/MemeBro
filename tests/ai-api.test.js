...
test('returns a valid JSON string with system and user keys', () => {
  ...
  assert.ok(typeof parsed.system === 'string' && parsed.system.length > 0, 'system prompt must be a non-empty string');
  assert.ok(typeof parsed.user === 'string' && parsed.user.length > 0, 'user message must be a non-empty string');
});
...
test('omits format line and uses suggested_format schema when memeFormat is not provided', () => {
  ...
  assert.ok(!user.includes('Format:'), 'user message must not include Format: when memeFormat is omitted');
  assert.ok(user.includes('suggested_format'), 'output schema must include suggested_format when no format is given');
});
