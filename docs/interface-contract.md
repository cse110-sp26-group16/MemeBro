...
| `memebro:favorites`        | `string[]`          | Array of favorited template ids. Gallery and search cards read/write. |
...
| `js/api/storage.js`     | `localStorage` wrapper                                                | `getMemes(): Meme[]`, `saveMeme(meme: Meme): void`, `deleteMeme(id: string): void`, `getRecentTemplates(): Template[]`, `recordRecentTemplate(template: Template): void`, `getTheme(): 'light'\|'dark'`, `setTheme(theme): void`, `getFavorites(): string[]`, `isFavorite(id): boolean`, `toggleFavorite(id): boolean` |
